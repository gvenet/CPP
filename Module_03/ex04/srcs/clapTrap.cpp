...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/06/15 12:30:07 by gvenet            #+#    #+#             */
/*   Updated: 2021/06/15 12:30:08 by gvenet           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
clapTrap::clapTrap(int hp, int maxHp, int energy, int maxEnergy, int level, int meleeAtt, int rangeAtt, int armorDmgRed, std::string const name, std::string type) : _hp(hp), _maxHp(maxHp), _energy(energy), _maxEnergy(maxEnergy), _level(level), _name(name), _meleeAttDamage(meleeAtt), _rangedAttDamage(rangeAtt), _armorDamageReduction(armorDmgRed), _type(type)
{
	srand((unsigned)time(NULL));
...
int clapTrap::getMeleeAttDmg(void) const
{
	return this->_meleeAttDamage;
}

int clapTrap::getRangedAttDamage(void) const
{
	return this->_rangedAttDamage;
}
